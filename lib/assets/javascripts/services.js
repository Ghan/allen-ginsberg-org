  ...
  factory('worksIndexService', ['$http', function($http) {
    return{
      ...
      getData: function(token, type, callback){
        $http.get(appPath+'locomotive/api/content_types/published_work/entries.json?works_index=true&work_type='+type+'&auth_token='+token, {cache: true}).success(function(data) {
          // prepare data here
          data = data.data;
          // console.log(data);
  ...
  factory('similarWorksService', ['$http', function($http) {
    return{
      ...
      getData: function(token, geo, notable, date, callback){
        $http.get(appPath+'localhost:8080/locomotive/api/content_types/published_work/entries.json?&similar_work=true&geo='+geo+'&notable='+notable+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
  ...
  factory('similarArchiveItemsService', ['$http', function($http) {
    return{
      ...
      getData: function(token, id, geo, notable, misc, date, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?&similar_archive_items=true&geo='+geo+'&notable='+notable+'&misc='+misc+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          for (var i = data.length - 1; i >= 0; i--) {
            if(data[i].slug == id){
