      ...
      getData: function(token, id, callback){
        $http.get(appPath+'locomotive/api/content_types/published_work/entries/'+id+'.json?&auth_token='+token, {cache: true}).success(function(data) {
          ...
          var reformattedData = {
            ...
            "image" : '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+data.image+'&container=focus&resize_w=250&refresh=2592000" title="'+data.content.name+'"/>'
          }
        ...
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?archive_index=true&auth_token='+token, {cache: false}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].original_file.indexOf("blank.png") != -1 && (data[i].archive_type == "document" || data[i].archive_type == "lecture")){
              ...
              data[i].image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+data[i].original_file+'&container=focus&resize_w=250&refresh=2592000" title="'+data[i].title+'"/>';
            } else {
              data[i].image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+data[i].original_file+'&container=focus&resize_w=250&refresh=2592000" title="'+data[i].title+'"/>';
            }
      ...
      getData: function(token, id, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries/'+id+'.json?&auth_token='+token, {cache: true}).success(function(data) {
          ...
          if(data.image.indexOf("blank.png") != -1){
            ...
            reformattedData.image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+data.image+'&container=focus&resize_w=1200&refresh=2592000" alt="'+data.content.title+'"/>';
          }
          callback(reformattedData);
        });
      }
