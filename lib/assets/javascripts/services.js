...
angular.module('gApp.services', []).
  ...
  factory('archiveIndexService', ['$http', function($http) {
    return{
      ...
      getData: function(token, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?archive_index=true&auth_token='+token, {cache: false}).success(function(data) {
          // prepare data here
          data = data.data;
          var fixedData = { 
              "photography":[],
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].original_file.indexOf("blank.png") != -1 && (data[i].archive_type == "document" || data[i].archive_type == "lecture")){
              ...
              data[i].image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].original_file+'&container=focus&resize_w=250&refresh=2592000" title="'+data[i].title+'"/>';
            } else {
              data[i].image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].original_file+'&container=focus&resize_w=250&refresh=2592000" title="'+data[i].title+'"/>';
            }
      ...
      getData: function(token, id, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries/'+id+'.json?&auth_token='+token, {cache: true}).success(function(data) {
          ...
          if(data.image.indexOf("blank.png") != -1){
            ...
            reformattedData.image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data.image+'&container=focus&resize_w=1200&refresh=2592000" alt="'+data.content.title+'"/>';
          }
          callback(reformattedData);
        });
      }
    ...
    return{
      ...
      getData: function(token, callback){
        $http.get(appPath+'locomotive/api/content_types/links/entries.json?links_page=true&auth_token='+token, {cache: true}).success(function(data) {
          // /locomotive/api/pages/521cd5fee1d12ab458000005.json
          // console.log(data.cache);
          data = data.data;
          var fixedData = {};
          for (var i = data.length - 1; i >= 0; i--) {
        ...
        $http.get(appPath+'/locomotive/api/content_types/published_work/entries.json?&similar_work=true&geo='+geo+'&notable='+notable+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].imageThumb.indexOf("blank.png") != -1){
              ...
              imageURL = "https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url="+s3imgURL+data[i].imageThumb+"&container=focus&resize_w=100&refresh=2592000";
            }
        ...
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?&items_in_archive='+items+'&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].image.indexOf("blank.png") == -1){
              imgURL = '<a href="#/archive/'+data[i].archive_type+'/'+data[i].slug+'" title="'+data[i].title+' ('+data[i].archive_type+')"><img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].image+'&container=focus&resize_w=100&resize_h=100&refresh=2592000" alt="'+data[i].title+' ('+data[i].archive_type+')" /></a>'
            } else {
              imgURL = '<a href="#/archive/'+data[i].archive_type+'/'+data[i].slug+'" title="'+data[i].title+' ('+data[i].archive_type+')"><img src="http://allen-ginsberg-org.herokuapp.com/assets/blank.png" alt="'+data[i].title+' ('+data[i].archive_type+')" /></a>'
            }
      ...
      getData: function(token, id, geo, notable, misc, date, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?&similar_archive_items=true&id='+id+'&geo='+geo+'&notable='+notable+'&misc='+misc+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].image.indexOf("blank.png") != -1){
              data[i].similarArchiveItem = '<a href="#/archive/'+data[i].archive_type+'/'+data[i].slug+'" title="'+data[i].title+' ('+data[i].archive_type+')"><img src="http://allen-ginsberg-org.herokuapp.com/assets/blank.png" alt="Missing Image" />';
            } else {
              data[i].similarArchiveItem = '<a href="#/archive/'+data[i].archive_type+'/'+data[i].slug+'" title="'+data[i].title+' ('+data[i].archive_type+')"><img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].image+'&container=focus&resize_w=100&resize_h=100&refresh=2592000" alt="'+data[i].title+' ('+data[i].archive_type+')" /></a>';
            }
          };
          callback(data);
        });
        ...
        $http.get(appPath+'locomotive/api/content_types/published_work/entries.json?&published_in='+items+'&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            if(data[i].imageThumb.indexOf("blank.png") != -1){
              ...
              data[i].publishedInImage = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].imageThumb+'&container=focus&resize_w=200&refresh=2592000" alt="{{p.name}}" />';
            }
          };
