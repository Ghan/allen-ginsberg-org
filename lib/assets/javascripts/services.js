...
angular.module('gApp.services', []).
      ...
      getData: function(token, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/archive_items/entries.json?archive_index=true&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].file_slash_image.indexOf("blank.png") != -1 && (data[i].archive_type == "document" || data[i].archive_type == "lecture")){
      ...
      getData: function(token, id, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/archive_items/entries/'+id+'.json?&auth_token='+token, {cache: true}).success(function(data) {
          ...
          if(data.image.indexOf("blank.png") != -1){
            ...
            reformattedData.image = "<img src='http://allen-ginsberg-org.herokuapp.com"+data.image+"' />";
          }
          callback(reformattedData);
        });
      }
    ...
    return{
      ...
      getData: function(token, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/links/entries.json?links_page=true&auth_token='+token, {cache: true}).success(function(data) {
          // http://localhost:8080/locomotive/api/pages/521cd5fee1d12ab458000005.json
          // console.log(data.raw_template);
          data = data.data;
          console.log(data);
          var fixedData = {};
          for (var i = data.length - 1; i >= 0; i--) {
  ...
  factory('similarWorksService', ['$http', function($http) {
    return{
      ...
      getData: function(token, geo, notable, date, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/published_work/entries.json?&similar_work=true&geo='+geo+'&notable='+notable+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
  ...
  factory('itemsInArchiveService', ['$http', function($http) {
    return{
      ...
      getData: function(token, items, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/archive_items/entries.json?&items_in_archive='+items+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
  ...
  factory('similarArchiveItemsService', ['$http', function($http) {
    return{
      ...
      getData: function(token, id, geo, notable, misc, date, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/archive_items/entries.json?&similar_archive_items=true&geo='+geo+'&notable='+notable+'&misc='+misc+'&date='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          for (var i = data.length - 1; i >= 0; i--) {
            if(data[i].slug == id){
  ...
  factory('publishedInService', ['$http', function($http) {
    return{
      ...
      getData: function(token, items, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/published_work/entries.json?&published_in='+items+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
  ...
  factory('atThisTimeService', ['$http', function($http) {
    return{
      ...
      getData: function(token, date, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/life_events/entries.json?&at_this_time='+date+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
  ...
  factory('otherClassService', ['$http', function($http) {
    return{
      ...
      getData: function(token, classId, callback){
        $http.get('http://localhost:8080/locomotive/api/content_types/archive_items/entries.json?&other_class_list='+classId+'&auth_token='+token, {cache: true}).success(function(data) {
          callback(data);
        });
      }
    }
