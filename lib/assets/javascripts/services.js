...
angular.module('gApp.services', []).
  ...
  factory('linksService', ['$http', function($http) {
    return{
      name: 'Links Service',
      getData: function(token, callback){
        $http.get('http://localhost:8080/locomotive/api/pages/521cd5fee1d12ab458000005.json?auth_token='+token, {cache: true}).success(function(data) {
          // console.log(data.cache); 
          console.log(data.raw_template);
          data = data.raw_template.links;
          console.log(data);
          var fixedData = {};
          for (var i = data.length - 1; i >= 0; i--) {
  ...
  factory('similarArchiveItemsService', ['$http', function($http) {
    return{
      ...
      getData: function(token, geo, notable, misc, date, callback){
