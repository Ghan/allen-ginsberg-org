      ...
      getData: function(token, id, callback){
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries/'+id+'.json?&auth_token='+token, {cache: true}).success(function(data) {
          ...
          if(data.image.indexOf("blank.png") != -1){
            ...
            reformattedData.image = '<img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url=https://allenginsberg.s3.amazonaws.com'+data.image+'&container=focus&resize_w=1200&refresh=2592000" alt="'+data.content.title+'"/>';
          }
          callback(reformattedData);
        });
      }
