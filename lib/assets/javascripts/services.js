        ...
        $http.get(appPath+'locomotive/api/content_types/archive_items/entries.json?&items_in_archive='+items+'&auth_token='+token, {cache: true}).success(function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            if(data[i].image.indexOf("blank.png") == -1){
              imgURL = '<a href="#/archive/'+data[i].archive_type+'/'+data[i].slug+'" title="'+data[i].title+' ('+data[i].archive_type+')"><img src="https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+s3imgURL+data[i].image+'&container=focus&resize_w=100&refresh=2592000" alt="'+data[i].title+' ('+data[i].archive_type+')" /></a>'
            } else {
